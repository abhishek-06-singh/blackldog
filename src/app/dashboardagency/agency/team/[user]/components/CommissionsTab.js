      ...
      {currentItems.map((c) => (
        ...
        >
          ...
          <td className="p-2 sm:p-4">
            <span
              className={`inline-flex bg-cartdbg items-center rounded-full px-3 py-1 text-xs font-medium 
                ${
                  c.payoutStatus === 'Paid'
                    ? 'border border-green-100 text-green-700'
                    : 'border border-status-pending-in-review-500 text-status-pending-in-review-700'
                }`}
            >
              {c.payoutStatus}
            </span>
