...
const statsData = [
  ...
  },
  {
  ...
  },
   {
    ...
    title: 'Commission Earnings',
    value: '$25,400',
    icon: <Trash className="w-6 h-6 text-black-400" />,
  },
]

export default function StatsCards_List({ data = statsData }) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mt-3 ml-5 md:ml-10 pr-5 md:pr-10">
      {data.map((stat, index) => (
        <motion.div
          key={stat.id}
          ...
          className="flex flex-col items-start p-3 bg-cardbg shadow-sm rounded-2xl border border-bordercolor hover:shadow-lg transition-shadow"
        >
