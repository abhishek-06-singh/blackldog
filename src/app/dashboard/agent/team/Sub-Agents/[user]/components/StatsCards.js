...
export default function StatsCards({ data = statsData }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-3 pr-2 md:pr-6 lg:pr-10">
      {data.map((stat, index) => (
        <motion.div
          key={stat.id}
          ...
          className=" border-bordercolor flex flex-col items-start p-4 bg-cardbg shadow-sm rounded-2xl border hover:shadow-lg transition-shadow"
        >
