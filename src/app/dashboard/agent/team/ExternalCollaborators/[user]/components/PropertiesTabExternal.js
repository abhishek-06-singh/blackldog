...
export default function PropertiesTabExternal({ decodedUser }) {
    ...
    <div className="mt-8">
      ...
      <div className="bg-cardbg overflow-x-auto rounded-xl border border-bordercolor shadow-md">
        <table className="min-w-full text-left table-auto">
          <thead className="bg-cardbg whitespace-nowrap text-textplaceholder text-sm font-thin">
            <tr>
              {/* No checkbox column */}
          ...
          <tbody className="divide-y divide-bordercolor whitespace-nowrap text-xs sm:text-sm text-textnormal">
            {currentItems.map((member) => (
              <tr key={member.id} className="hover:bg-boxbg transition">
                ...
                <td className="p-4">
  <div className="flex text-center flex-row items-center justify-start gap-2">
    <img
      src={member.assignedBy.avatar}
      alt={member.assignedBy.name}
      className="w-6 h-6 rounded-full object-cover"
    />
    <span className="text-sm">{member.assignedBy.name}</span>
  </div>
</td>
      ...
      {/* Pagination */}
      <div className="flex items-center justify-between mt-4 text-sm text-textnormal">
        <p>
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
            <button
              ...
              className={`px-3 py-1 rounded-lg border ${
                currentPage === page
                  ? 'bg-black-500 text-textnormal border-textnormal'
