...
export default function DealHistoryTable() {
  ...
  const handleRowClick = (title) => {
    router.push(`/dashboardindividualagent/subagentindividualagent/deals/deal-history/${title}`)
  }
            ...
            {currentItems.map((item) => (
              <tr key={item.id} className="hover:bg-boxbg transition">
                ...
                <td
                  ...
                  className="cursor-pointer p-4 text-sm font-medium whitespace-nowrap text-textnormal"
                >
                  {item.title}
                </td>
